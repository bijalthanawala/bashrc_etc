...
function mkdircd() {
    # Create a new directory and cd to it instantly
    ...
    if [ -z "${new_dir}" ]; then
        echo "Specify a new directory to create and change to."
        return
    fi
    ...
    # Perform verbose
    if [ -d "${new_dir}" ] ; then
        echo "This directory already exists: ${new_dir}"
    else
    ...
    result=$?
    if [ $result -ne 0 ] ; then
        echo "Failed to create new directory: ${new_dir}"
        return
    fi
    ...
    echo "Type popd to return back to $(pwd)"
    cd ${new_dir}
...
function initialize() {
    ...
    alias ${script_alias}="vim ${this_script_fullname}; source ${this_script_fullname} --verbose"
